...
- **Viz rendering layer**: `public/icc-viz.js` — vanilla-JS port of profiletool's React viz components (GraphSvg / RasterCanvas / Collapsible / TagEvaluator / the TagVisuals dispatcher / rasterDecode / colors). Exposes `window.IccViz.{enumerate, renderTagDetail}`. Built entirely via DOM nodes + `textContent` (never `innerHTML` for profile-derived strings such as named-colour / ink names), so untrusted profile text can't become markup — same hygiene as `escapeHtml`. The viewer's UI chrome (section titles like *Curves* / *Input curves* / *CLUT* / *Gamut* / *Evaluate*, the evaluator's *Input* / *Output* / *Human* / *Norm* / *Float* / *Grid* labels, etc.) **is** localized: `icc-viz.js` looks each label up via a small `tr(key, fallback)` helper that delegates to the host page's `window.t` (keys prefixed `icc_viz_`), falling back to the English default if `t` is unavailable. Only profile-derived text (ink names, the `Describe()` dump, header values) stays verbatim. The Tags-tab column headers (*Tag Name* / *ID* / *Type* / *Offset* / *Size* / *Pad*), the *Profile ID* label and "No tags found." live in `index.html` (`_renderIccTags` / `_renderIccHeader`) and use `t()` directly with `icc_col_*` / `icc_no_tags` / `icc_array_of` / `icc_profile_id` keys; `onLangChange` re-renders the open viewer so a live language switch updates them (it collapses any expanded tag row — its viz labels re-localize on re-expand).
...
**CxF/X-3** (ISO 17972-3): `parseCxF` uses `DOMParser` + `getElementsByTagNameNS('*', localName)` (prefix-agnostic — real files use the `cc:` prefix, not the default namespace). Per `Object` it reads `Name`→`SAMPLE_NAME`, first `ColorCIELab`→`LAB_*`, first `ReflectanceSpectrum`→`xxx_NM` (0–1 reflectance; `StartWL` from the spectrum element, falling back to the referenced `ColorSpecification`'s `WavelengthRange` — CxF/X-4 files only declare it there), and `ColorCMYK`/`ColorCMYKPlusN`/`ColorRGB`→device colorants (0–100). `_buildCxF()` is the writer (full X-3, `cc:` prefix), wired into both `exportData` paths as the `cxf` format.

**CxF/X-4** (ISO 17972-4, spot ink characterisation): handled inside `parseCxF`. When a `SpotInkCharacterisation` custom resource is present, its `MeasurementSet`/`Measurement` elements (attrs `Background`, `TintLevel`, `ObjectRef`) replace the plain object walk: the spot ink becomes a device colorant column named from `SpotInkName` (value = tint level 0–100), and with two measurement sets (full X-4) a second `PROCESS_BLACK` column is 0 (substrate set) / 100 (any `Background` matching /black/i). `parseCxF` returns `cxfVariant` (`'CxF/X-4'` two sets; `'CxF/X-4a'` one set with intermediate tints; `'CxF/X-4b'` one set, 0/100 only; null = plain X-3) which `processFileText` shows as the type label (persisted via `state[slot].typeLabel`) and converts to `state[slot].noModel = true`. `noModel` keeps the full table/plot/TV/Extract/Compare feature set but skips polynomial-model fitting (`buildGamutCache`), the gamut-shell panel controls, and the Estimate section — 1–2-colorant tint ramps can't support the model. The `cxfx4_notice` i18n key explains this in the file panel.
