 ...
 *
 * Vendored verbatim from profiletool's validator-wasm/plot-wrapper.cpp (the
 * app-side embind binding) and compiled into chardata's icc-viewer WASM target
 * alongside wrapper.cpp, so window.IccViewer exposes both the profile-dump
 * functions (validateProfile/describeTag) and these visualization functions from
 * one module. The IccVizModel engine itself is the shared ~/code/profiletool/iccviz
 * tree (referenced by CMakeLists as ICCVIZ_ROOT — not forked here).
 *
 * Exposes IccVizModel (profiletool/iccviz/IccVizModel.cpp) to the browser:
